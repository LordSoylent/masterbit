...
const char * const BITCOIN_CONF_FILENAME = "masterbit.conf";
const char * const BITCOIN_PID_FILENAME = "masterbitd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "masterbit" is a composite category enabling all MasterBit-related debug output
            if(ptrCategory->count(string("masterbit"))) {
...
#else
    const char* pszModule = "masterbit";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MasterBitCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MasterBitCore
    // Mac: ~/Library/Application Support/MasterBitCore
    // Unix: ~/.masterbitcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MasterBitCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/MasterBitCore";
#else
    // Unix
    return pathRet / ".masterbitcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty masterbit.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override masterbit.conf
